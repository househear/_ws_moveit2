...
DEVICE_CONFIG = {
    'camera_panda': {'topic_name': 'camera', 'timestep': 16}
    }
...
class WebotsRoboticArmNode(WebotsNode):
    ...
    def __init__(self, name, args, prefix='', controller_name='', device_config = ''):
        super().__init__(name, args)
        #self.start_joint_state_publisher(name)
        ...
        self.__device_config_param = self.declare_parameter('device_config', device_config)
...
def main(args=None):
    ...
    robotic_arm = WebotsRoboticArmNode(webots_robot_name, args=args, prefix='', controller_name='')
